...
-- Lucky's Wardrobe: expansions and kinds of set typed into a set list's search
-- box. Typing "tww" into a Sets tab narrows it to The War Within instead of
-- hunting for a set whose name holds those three letters, which no set does,
-- and "tww pvp" or "tww raid" narrows it again to where the sets came from.
--
...
-- What kind of set to keep. These are the ones every list can answer for
-- itself: PvP and its opposite, the way Blizzard's own two boxes split the
-- collection, and raid tier. The Sources filter sorts sets several other ways,
-- but only on the lists Blizzard dates, and a word that worked on one tab and
-- emptied another would read as broken.
local KINDS = { pvp = true, pve = true, raid = true }
...
--- Takes the kind off whichever end it was typed on, so "pvp tww" and "tww pvp"
--- read the same and an expansion named in several words still has its own end
--- of the text to itself. Hands back what is left and the kind that came off.
local function takeKind(query)
    local head, rest = query:match("^(%a+) (.+)$")
    if head and KINDS[head] then return rest, head end

    local body, tail = query:match("^(.+) (%a+)$")
    if tail and KINDS[tail] then return body, tail end

    if KINDS[query] then return "", query end
    return query, nil
end

--- What a search box's whole text narrows a set list to, as
--- { expansionID = , kind = }, either of which may be missing. nil where the
--- text narrows nothing and is the set name search it has always been.
---
...
function SetSearch.Parse(text, expansionNames)
    ...
    local rest, kind = takeKind(query)
    local expansionID
    if rest ~= "" then
        ...
        -- one, and a kind alone is not enough to claim a search that also asked
        -- for something this cannot answer.
        if expansionID == nil then return nil end
    end

    if expansionID == nil and kind == nil then return nil end
    return { expansionID = expansionID, kind = kind }
end
...
--- with the rest.
---
--- isPvP and isRaid are the caller's own answers, because the two kinds of list
--- reach them differently: a set the client dates is read through the Sources
--- classifier, while a bundled one is read off Wowhead's source bits and the
--- difficulty the snapshot names it by.
function SetSearch.Matches(narrowedTo, expansionID, isPvP, isRaid)
    ...
    local kind = narrowedTo.kind
    if kind == "pvp" then return isPvP == true end
    -- Everything that is not PvP, raid tier among it, the way Blizzard's own
    -- pair of boxes divides the collection between them.
    if kind == "pve" then return isPvP ~= true end
    if kind == "raid" then return isRaid == true end
    return true
end
...
function SetSearch:Init()
    ...
    local function remember(parsed)
        ...
        LuckysWardrobe.DevLog("Set search: expansion=" .. tostring(parsed and parsed.expansionID)
            .. " kind=" .. tostring(parsed and parsed.kind))
        redrawSetLists()
    end
